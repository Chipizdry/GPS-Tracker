...
var pub_int = []
var pub_int_cnt = 0
var text_area_pub_int = []
var text_area_pub_int_cnt = 0
var select_pub_int = []
var select_pub_int_cnt = 0
...
function content_item(item, i){
    ...
    if(item.html == "textarea"){
        ...
        if (item.readOnly == "true") {
          content += "<textarea class=\"pure-u-24-24\" rows=\"13\" maxlength=\"2000\" readOnly=\"" + item.readOnly + "\" id=\"" + item.id + "\" oninput=\"data('" + item.type + "', this.id, this.value, '" + item.label + "', " + page + ", " + i + ")\">" + item.value + "</textarea>";
        }
        else {
          content += "<textarea class=\"pure-u-24-24\" rows=\"13\" maxlength=\"2000\" id=\"" + item.id + "\" oninput=\"data('" + item.type + "', this.id, this.value, '" + item.label + "', " + page + ", " + i + ")\">" + item.value + "</textarea>";
        }
        content += "<label id=\"" + item.id + "-val\">" + item.label + " "

        //window.alert(item.readonly);
        //pubTextArea(item.id, item.value)
        text_area_pub_int[text_area_pub_int_cnt] = setInterval(() => {
            pub(item.id, item.value)
        }, 500);
        text_area_pub_int_cnt++
        return content
    }
...
function data(type, id, value, label, page, i){
    ...
    if (type == "text" || type == "password") {
      document.getElementById(id + "-val").innerHTML = label + "&nbsp(" + value.length + ")";
    }
    ...
    var value_r = value.replace(/(\r\n|\n|\r)/gm, '&#13;&#10;');
    obj.content[page][i].value = value_r;
    send(id, value_r)
}
...
function pub(id, unit){
    var xhr = new XMLHttpRequest();
    var xhr_send = new Boolean(false);
    xhr.open('GET', '/pub?' + id, true);
    xhr.onload = function () {
        if(document.getElementById(id).innerHTML != xhr.responseText)
        {
          //alert(document.getElementById(id).innerHTML.substr(document.getElementById(id).innerHTML.length - 3));
          var str12 = xhr.responseText.replace(/(\r\n|\n|\r)/gm, '&#13;&#10;');
          //alert(xhr.responseText.length+" A: "+str12.length+" B: "+document.getElementById(id).innerHTML.length);
          //alert(str12.length);
          //alert(findDiff(document.getElementById(id).innerHTML, xhr.responseText));
...
function pubSelect(id, text){
    var xhr = new XMLHttpRequest();
    xhr.open('GET', '/pub?' + id, true);
    xhr.onload = function () {
      document.getElementById(id).innerHTML = text;
      var xa = document.getElementById(id);
